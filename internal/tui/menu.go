...
func RunDaemonMenu() (string, error) {
	...
	fmt.Println()
	fmt.Println(ui.Dim.Render("  🗑️  Remove from boot:  Uninstall the system service so the daemon no"))
	fmt.Println(ui.Dim.Render("                        longer starts automatically on boot."))
	fmt.Println()
	fmt.Println(ui.Dim.Render("  💀 Kill all:          Force-kill every opencrons process on this machine."))
	fmt.Println(ui.Dim.Render("                        Use when Stop hangs or processes are orphaned."))
	fmt.Println()
	...
	options = append(options, huh.NewOption("🗑️  Remove from boot", "uninstall"))
	options = append(options, huh.NewOption("💀 Kill all processes", "killall"))
